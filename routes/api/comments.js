...
router.post("/:commentId/like", verifyToken, async (req, res) => {
  ...
  if (comment.dislikes.map((objId) => objId.toString()).includes(req.userId)) {
...
// Dislike a comment
router.post("/:commentId/dislike", verifyToken, async (req, res) => {
  const comment = await commentSchema.findById(req.params.commentId);
  if (!comment) {
    return res.status(404).json({ message: "Comment not found" });
  }
...
router.post("/:commentId/undislike", verifyToken, async (req, res) => {
  const comment = await commentSchema.findById(req.params.commentId);
  if (!comment) {
    return res.status(404).json({ message: "Comment not found" });
  }
